...
Push-Location $root
try {
    npm run package:win --workspace=@codex-assistant/desktop -- --config.electronDist=../../node_modules/electron/dist
...
$manifest = [ordered]@{ product = 'CodexAssistant'; version = $version; installer = [ordered]@{ file = (Split-Path $target -Leaf); sha256 = $hash; sizeBytes = $size }; signatureStatus = [string]$signature.Status }
