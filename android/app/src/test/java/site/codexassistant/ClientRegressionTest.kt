...
class ClientRegressionTest {
    ...
    private val manifest = """{"product":"CodexAssistant","protocolVersion":"codex-assistant.v3","downloads":{"android":{"versionName":"2.0.3","versionCode":6,"url":"https://server.example.com/app.apk"},"windows":{"url":"https://server.example.com/app.exe"}}}"""
    ...
    @Test fun updateRejectsWrongProtocolAndUnsafeDownloads() {
        for (invalid in listOf(manifest.replace("codex-assistant.v3", "codex-assistant.v1"), manifest.replace("https://server.example.com/app.apk", "http://server.example.com/app.apk"))) {
