...
class ClientRegressionTest {
    @Test
    fun secondaryPagesReturnToSettingsAndHideRootNavigation() {
    ...
    @Test
    fun everyProtocolTaskStateCanBeFilteredUsingItsNotificationLabel() {
    ...
    @Test
    fun taskTimesConvertTheInstantToDeviceTimezoneIncludingDateAndOffset() {
        ...
        assertEquals(
            "2026-09-08 16:30:00 +00:00",
            formatTime("2026-09-08T16:30:00Z", java.time.ZoneId.of("UTC")),
        )
    ...
    private val manifest =
        """{"product":"CodexAssistant","protocolVersion":"codex-assistant.v3","downloads":{"android":{"versionName":"2.0.3","versionCode":6,"url":"https://server.example.com/app.apk"},"windows":{"url":"https://server.example.com/app.exe"}}}"""

    @Test
    fun updateRequestLeavesTheCallingUiThread() {
        var networkThread = ""
        val client =
                ...
                .addInterceptor(
                    Interceptor { chain ->
                        ...
                        Response.Builder()
                            ...
                            .code(200)
                            .message("OK")
                            .body(manifest.toResponseBody())
                            .build()
                    }
                )
                .build()
        Executors.newSingleThreadExecutor { runnable -> Thread(runnable, "test-ui") }
    ...
    @Test
    fun updateRejectsWrongProtocolAndUnsafeDownloads() {
        for (invalid in
            listOf(
    ...
    @Test
    fun actualWireSerializerSendsRequiredDefaultsAndOmitsOptionalNulls() {
        val auth =
            wireJson
                .parseToJsonElement(
                    wireJson.encodeToString(ClientAuthMessage(token = "test-token-123456"))
                )
                .jsonObject
        assertEquals(setOf("type", "protocolVersion", "token"), auth.keys)
        val subscription =
            wireJson
                .parseToJsonElement(wireJson.encodeToString(ClientSubscribeMessage(after = 42)))
                .jsonObject
    ...
    @Test
    fun notificationThrottlePreservesTheFinalStateAndAllStatusLabels() {
        val task =
            TaskSnapshot(
                "task",
                "任务名称",
        ...
        for (status in listOf("running", "completed", "failed", "needs_action")) assertNotEquals(
            "未知状态",
            statusLabel(status),
        )
    }
