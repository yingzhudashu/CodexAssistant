...
data class TaskSnapshot(
    ...
    init {
        require(status in setOf("running", "completed", "failed", "needs_action")) { "未知任务状态" }
    }
}

@Serializable
data class TraceContext(val traceId: String, val spanId: String, val parentSpanId: String? = null)
...
@Serializable
data class EventMessage(val type: String, val protocolVersion: String, val event: ServerEvent)
...
@Serializable
data class ClientSubscribeMessage(
...
@Serializable
data class ClientDetailMessage(
...
@Serializable
data class InteractionSubmit(
    val type: String = "interaction.submit",
    val protocolVersion: String = PROTOCOL_VERSION,
    val requestId: String,
    val threadId: String,
...
data class ResultMessage(
    ...
    init {
        require(status in setOf("started", "failed")) { "未知发送回执状态" }
    }
}

@Serializable
data class InteractionOption(val id: String, val label: String, val description: String? = null)
