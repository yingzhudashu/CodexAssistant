...
internal fun connectionStatusLabel(status: String): String =
    when (status) {
        "connecting" -> "连接中"
        "authenticating" -> "认证中"
        "subscribing" -> "同步中"
...
internal fun connectionSummary(state: TaskState): String =
    when {
...
internal fun backgroundSyncSummary(state: TaskState): String =
    when (state.backgroundSyncStatus) {
        "running" -> "后台同步服务运行中"
