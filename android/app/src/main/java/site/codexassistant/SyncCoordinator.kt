...
/** 应用级连接所有权：可见 Activity 和前台服务共享唯一 WebSocket。 */
class SyncCoordinator(context: Context) {
    ...
    private fun start() {
        ...
        if (credentials.token() == null) {
            mutableState.value =
                mutableState.value.copy(connectionStatus = "not_configured", error = "请先配置访问 Token")
            return
        }
        if (callback == null) {
            val listener =
        ...
        val initial =
            mutableState.value.copy(
                ...
                backgroundSyncStatus = background,
                connected = false,
            )
        mutableState.value = initial
        val next = TaskRepository(credentials, initial, traceLogger)
        repository = next
        job = scope.launch {
            next.stream().collect { incoming ->
                synchronized(this@SyncCoordinator) {
                    if (repository === next)
                        mutableState.value =
                            incoming.copy(
    ...
    private fun stopIfUnowned() {
        if (!visible && service == null) stop()
    }
    ...
    fun restart() {
        ...
        mutableState.value =
            TaskState(
                connectionEpoch = mutableState.value.connectionEpoch + 1,
                cursor = CredentialStore(appContext).cursor(),
                backgroundSyncStatus = background,
            )
        start()
    }
    ...
    fun requestDetail(threadId: String, cursor: String? = null): String =
        repository?.requestDetail(threadId, cursor) ?: error("连接不可用")

    fun sendMessage(threadId: String, text: String): String =
        repository?.sendMessage(threadId, text) ?: error("连接不可用")

    fun submitInteraction(
        requestId: String,
        threadId: String,
        value: kotlinx.serialization.json.JsonElement,
    ): Boolean = repository?.submitInteraction(requestId, threadId, value) == true
    ...
    fun refreshNow(): Boolean {
        ...
        if (!accepted)
            mutableState.value =
                mutableState.value.copy(isRefreshing = false, refreshError = "同步尚未启动，请稍后重试")
        return accepted
    }
}
