...
/** 连接生命周期串行化；网络与 Activity 信号统一由 SyncCoordinator 驱动。 */
...
internal constructor(
    ...
    constructor(
        ...
        initial: TaskState,
        traceLogger: TraceLogger = TraceLogger(),
    ) : this(
        credentials::token,
        credentials::serverBaseUrl,
        credentials::saveCursor,
        initial,
    ...
    fun refreshNow(): Boolean =
        synchronized(lock) {
            val callback = recovery ?: return@synchronized false
            callback(available, true, true)
            true
        }
    ...
    private fun send(payload: String): Boolean =
        synchronized(lock) { !closed && activeSocket?.send(payload) == true }
    ...
    fun requestDetail(threadId: String, cursor: String? = null): String {
        ...
        check(
            send(
                json.encodeToString(
                    ClientDetailMessage(requestId = id, threadId = threadId, cursor = cursor)
                )
            )
        ) {
            "连接不可用"
    ...
    fun sendMessage(threadId: String, text: String): String {
        ...
        check(
            send(
                json.encodeToString(
                    ClientSendMessage(requestId = id, threadId = threadId, text = text.trim())
                )
            )
        ) {
            "连接不可用"
    ...
    fun submitInteraction(requestId: String, threadId: String, value: JsonElement): Boolean =
        send(
            json.encodeToString(
                InteractionSubmit(requestId = requestId, threadId = threadId, value = value)
            )
        )
    ...
    fun stream(): Flow<TaskState> = callbackFlow {
        ...
        fun scheduleTraceUpload() {
            ...
            launch(Dispatchers.IO) {
                try {
                    delay(500) // 合并握手与突发事件的诊断，复用同一个上传任务。
                    val accessToken = token() ?: return@launch
                    val spans = traceLogger.pending()
                    if (spans.isEmpty()) return@launch
                    val body =
                        json
                            .encodeToString(TraceSpanBatch(spans = spans))
                            .toRequestBody("application/json".toMediaType())
                    val request =
                        Request.Builder()
                            .url("${baseUrl()}/codex-assistant/api/v3/traces/spans")
                            .header("Authorization", "Bearer $accessToken")
                            .post(body)
                            .build()
                    try {
                        traceClient.newCall(request).execute().use {
                            if (it.isSuccessful)
                                traceLogger.acknowledge(spans.map { span -> span.spanId }.toSet())
                        }
                    } catch (_: java.io.IOException) {}
                } finally {
                    synchronized(lock) { traceUploadRunning = false }
                }
            }
        }
        fun disconnected(reason: String) {
            ...
            state =
                state.copy(
                    connected = false,
                    connectionStatus = if (available) "reconnecting" else "offline",
                    error = reason,
            ...
            if (available && !permanentFailure && !closed) {
                ...
                state =
                    state.copy(
                        ...
                        retryAtEpochMs = System.currentTimeMillis() + wait,
                    )
                retryJob = launch {
                    delay(wait)
                    synchronized(lock) {
        ...
        connect = connect@{
            ...
            val request =
                Request.Builder()
                    .url(
                        baseUrl().replaceFirst(Regex("^http"), "ws") +
                            "/codex-assistant/api/v3/stream"
                    )
                    .build()
            socket =
                sockets.newWebSocket(
                    request,
                    object : WebSocketListener() {
                        fun valid(ws: WebSocket) =
                            !closed && generation == attemptGeneration && socket === ws
                        ...
                        override fun onOpen(webSocket: WebSocket, response: Response) {
                            synchronized(lock) {
                                ...
                                state =
                                    state.copy(connectionStatus = "authenticating", error = null)
                                publish()
                                traceLogger.event("android.sync.authenticate", connectionTraceId)
                                if (
                                    !webSocket.send(
                                        json.encodeToString(ClientAuthMessage(token = accessToken))
                                    )
                                )
                                    disconnected("认证发送失败")
                            }
                        ...
                        override fun onMessage(webSocket: WebSocket, text: String) {
                            synchronized(lock) {
                                ...
                                try {
                                    ...
                                    val messageVersion =
                                        messageObject["protocolVersion"]?.jsonPrimitive?.content
                                    if (messageVersion != PROTOCOL_VERSION) {
                                        permanentFailure = true
                                        state =
                                            state.copy(
                                                connected = false,
                                                connectionStatus = "protocol_error",
                                                error = "协议版本不受支持",
                                                retryAtEpochMs = null,
                                            )
                                        trySend(state)
                                    ...
                                    when (messageType) {
                                        "authenticated" -> {
                                            ...
                                            webSocket.send(
                                                json.encodeToString(
                                                    ClientSubscribeMessage(after = state.cursor)
                                                )
                                            )
                                            traceLogger.event(
                                                "android.sync.subscribe",
                                                connectionTraceId,
                                            )
                                            scheduleTraceUpload()
                                        }
                                            ...
                                            state =
                                                traceLogger.span(
                                                    ...
                                                    state.copy(
                                                        ...
                                                        lastTraceId = event.trace.traceId,
                                                    )
                                                }
                                            scheduleTraceUpload()
                                        ...
                                        "detail" -> {
                                            ...
                                            // 仅保留最近 20 个单页回执，完整分页历史由可见详情拥有。
                                            state =
                                                state.copy(
                                                    details =
                                                        (state.details - detail.threadId +
                                                                (detail.threadId to detail))
                                                            .entries
                                                            .toList()
                                                            .takeLast(20)
                                                            .associate { it.toPair() }
                                        ...
                                        "result" -> {
                                            val result = json.decodeFromString<ResultMessage>(text)
                                            // StateFlow 允许合并相邻状态；回执必须积累到状态中，不能只存“最后一条”。
                                            state =
                                                state.copy(
                                                    results =
                                                        (state.results +
                                                                (result.requestId to result))
                                                            .entries
                                                            .toList()
                                                            .takeLast(1000)
                                                            .associate { it.toPair() }
                                        ...
                                        "interaction.request" -> {
                                            val interaction =
                                                json.decodeFromString<InteractionRequest>(text)
                                            state =
                                                state.copy(
                                                    interactions =
                                                        state.interactions +
                                                            (interaction.requestId to interaction)
                                                )
                                            trySend(state)
                                        }
                                        "interaction.result" -> {
                                            val result =
                                                json.decodeFromString<InteractionResult>(text)
                                            state =
                                                state.copy(
                                                    interactions =
                                                        if (result.status == "failed")
                                                            state.interactions
                                                        else state.interactions - result.requestId,
                                                    interactionResults =
                        ...
                        override fun onClosing(webSocket: WebSocket, code: Int, reason: String) {
                            synchronized(lock) {
                                if (valid(webSocket) && !permanentFailure)
                                    disconnected("服务连接已关闭，正在恢复")
                            }
                        }
                ...
                if (!permanentFailure && !closed) {
                    if (foreground && networkChanged) {
                        ...
                        changed ||
                            networkChanged ||
                            foreground &&
                                (state.connected || socket == null || elapsed() >= deadline)
                    )
                        connect()
                }
                publish()
    ...
    /** 已排序列表中只移动变化项，避免每个事件对千条快照重新排序。 */
    private fun upsert(tasks: List<TaskSnapshot>, next: TaskSnapshot): List<TaskSnapshot> {
        val previous = tasks.indexOfFirst { it.id == next.id }
        if (previous >= 0 && tasks[previous] == next) return tasks
        val updated = tasks.toMutableList()
        if (previous >= 0) updated.removeAt(previous)
        var low = 0
        var high = updated.size
        while (low < high) {
            val middle = (low + high) ushr 1
            if (updated[middle].updatedAt >= next.updatedAt) low = middle + 1 else high = middle
        }
        updated.add(low, next)
        return updated
    }
}
