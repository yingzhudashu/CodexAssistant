...
it("corrects interrupted/notLoaded from an independent app-server using actual start evidence", async () => {
  ...
  const task: TaskSnapshot = { id: "thread", title: "Task", runtimeStatus: "notLoaded", status: "idle", activeFlags: [], latestTurn: { status: "interrupted" }, source: "thread", freshness: "fresh", plan: [], updatedAt: "2026-09-08T06:00:00Z", changedAt: "2026-09-08T06:00:00Z" };
  expect(applyLifecycle(task, await reader.read("thread", path))).toMatchObject({ status: "active", runtimeStatus: "notLoaded", latestTurn: { status: "inProgress" } });
  ...
  expect(applyLifecycle({ ...task, goal: { objective: "Goal", status: "paused", tokensUsed: 0, timeUsedSeconds: 0 } }, await reader.read("thread", path)).status).toBe("paused");
});
...
it("keeps the state transition time separate from later metadata updates", async () => {
  const { path, reader } = await fixture(event("task_started"));
  const task: TaskSnapshot = { id: "thread", title: "Task", runtimeStatus: "notLoaded", status: "idle", activeFlags: [], source: "thread", freshness: "fresh", plan: [], updatedAt: "2026-09-08T06:10:00Z", changedAt: "2026-09-08T06:10:00Z" };
  const result = applyLifecycle(task, await reader.read("thread", path));
