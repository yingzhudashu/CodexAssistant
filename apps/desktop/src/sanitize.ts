...
const SECRET =
  /(?:bearer\s+|(?:api[_-]?key|token|secret|password)\s*[:=]\s*)[A-Za-z0-9._~+\-/=]{8,}/gi;
const KEY_SHAPE =
  /\b(?:sk-[A-Za-z0-9_-]{12,}|gh[pousr]_[A-Za-z0-9_]{20,}|AIza[A-Za-z0-9_-]{20,}|eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,})\b/g;
const WINDOWS_PATH = /(?:[A-Za-z]:\\|\\\\)[^\s"'<>]+/g;
const UNIX_PATH =
  /\/(?:Users|home|var|tmp|workspace|workspaces|mnt)\/[^\s"'<>]+/gi;
...
export function normalizeTurn(value: unknown): LatestTurn | undefined {
  ...
  const status: TurnStatus | undefined =
  ...
  return {
    ...
    ...(typeof row.durationMs === "number" && Number.isFinite(row.durationMs)
      ? { durationMs: Math.max(0, Math.trunc(row.durationMs)) }
      : {}),
    ...(error
      ? {
          error: {
            code: sanitizeText(error.code || "TURN_FAILED", 80).replace(
              /[^A-Za-z0-9._:-]/g,
              "_",
            ),
            message: sanitizeText(error.message || "执行失败", 200),
          },
        }
      : {}),
  };
