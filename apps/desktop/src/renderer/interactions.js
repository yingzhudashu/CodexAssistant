  ...
  function bind(root, submit) {
    root.querySelectorAll("[data-interaction]").forEach((form) => {
      const request = requests.find(
        (r) => r.requestId === form.dataset.interaction,
      );
      const save = () => {
        ...
        (request.questions || []).forEach((q, i) => {
          ...
          const selected = [
            ...form.querySelectorAll(`[data-question="${i}"]:checked`),
          ].map((el) => q.options[Number(el.dataset.option)].label);
      ...
      const send = async (value) => {
        ...
        try {
          ...
          if (result.status === "failed")
            errors.set(request.requestId, result.error);
          else {
            drafts.delete(request.requestId);
            errors.delete(request.requestId);
          }
        } catch {
          errors.set(request.requestId, "结果尚未确认，请核实后重试");
        } finally {
      ...
      form.addEventListener("submit", (event) => {
        ...
        if (
          request.questions.some(
            (q) => q.required && !answers[q.id]?.some((x) => x.trim()),
          )
        ) {
          errors.set(request.requestId, "请完成所有必填问题");
          window.dispatchEvent(new Event("interactions-updated"));
          return;
        }
        void send({
      ...
      form
        ...
        .forEach((button) =>
          button.addEventListener(
            "click",
            () => void send({ decision: button.dataset.decision }),
          ),
        );
      form
        .querySelector("[data-cancel-request]")
        .addEventListener("click", () => void send({ cancel: true }));
    });
  }
  return { render, bind };
})();
