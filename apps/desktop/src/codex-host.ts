...
function frame(value: unknown): Buffer {
  ...
  if (payload.length > MAX_FRAME_BYTES)
    throw new Error("CODEX_HOST_REQUEST_TOO_LARGE");
  ...
  try {
    const { stdout } = await execFileAsync(
      ...
      [
        "-NoProfile",
        "-Command",
        "Get-ChildItem -Name '\\\\.\\pipe\\' | Where-Object { $_ -like 'codex-browser-use-*' }",
      ],
      { timeout: 2_000, windowsHide: true, maxBuffer: 64 * 1024 },
    );
    return [
      ...new Set(
        stdout
          .split(/\r?\n/)
          .map((name) => name.trim())
          .filter((name) => /^codex-browser-use-[a-f0-9-]+$/.test(name)),
      ),
    ].map((name) => `\\\\.\\pipe\\${name}`);
  } catch {
    return [];
...
// 使用Desktop随附codex-app-tools的帧格式；这是版本相关的本机私有接口。
export class NativeHostPipe {
  ...
  async request(
    ...
    if (this.#closed || socket.destroyed)
      throw new Error("CODEX_HOST_PIPE_CLOSED");
    return new Promise((resolve, reject) => {
      const timer = setTimeout(
        () => this.#disconnect(new Error("CODEX_HOST_TIMEOUT")),
        timeoutMs,
      );
      this.#pending.set(id, { resolve, reject, timer });
      socket.write(payload, (error) => {
        if (error) this.#disconnect(new Error("CODEX_HOST_PIPE_CLOSED"));
      });
    });
  }
  ...
  async #connect(): Promise<net.Socket> {
    ...
    this.#connecting = new Promise<net.Socket>((resolve, reject) => {
      ...
      this.#socket = socket;
      const timer = setTimeout(
        () => socket.destroy(new Error("CODEX_HOST_CONNECT_TIMEOUT")),
        2_000,
      );
      socket.once("connect", () => {
        clearTimeout(timer);
  ...
  #onData(chunk: Buffer): void {
    ...
    while (this.#buffer.length >= 4) {
      ...
      if (!length || length > MAX_FRAME_BYTES) {
        this.#disconnect(new Error("CODEX_HOST_INVALID_RESPONSE"));
        return;
      }
...
export class CodexDesktopHost {
  ...
  constructor(
    private readonly candidates: () => Promise<string[]> = candidatePipes,
  ) {}
  ...
  async sendMessage(threadId: string, text: string): Promise<void> {
    ...
    try {
      ...
      if (record(result) && result.success === false)
        throw new Error("CODEX_DESKTOP_SEND_REJECTED");
      if (
        !record(result) ||
        result.success !== true ||
        !Array.isArray(result.contentItems) ||
        !result.contentItems.every(
          (item) =>
            record(item) &&
            ...
            try {
              ...
              if (
                ...
                result.tools.some(
                  (tool) =>
                    record(tool) &&
                    tool.name === "send_message_to_thread" &&
                    tool.namespace === "codex_app",
                )
              )
                return pipe;
            } catch {
              /* Only discovery is read-only. A write is never retried. */
            }
            pipe.close();
            this.#connections.delete(pipe);
            return undefined;
