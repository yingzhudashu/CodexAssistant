...
contextBridge.exposeInMainWorld("codexAssistant", {
  ...
  onSyncStatus: (listener: (status: "connecting" | "syncing" | "connected" | "offline") => void) => {
    const handler = (_event: Electron.IpcRendererEvent, status: "connecting" | "syncing" | "connected" | "offline") => listener(status);
