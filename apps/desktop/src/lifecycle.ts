...
export type Lifecycle = {
  ...
  turn: LatestTurn;
  updatedAt: string;
  /** rollout 文件最近被写入的时间，仅用于本地新鲜度判断 */ evidenceAt: string;
};
type Cursor = {
  path: string;
  identity: string;
  ...
  pending: Buffer;
  latest?: Lifecycle;
  search?: { end: number; position: number; suffix: Buffer };
};
const CHUNK = 64 * 1024;
...
export function lifecycleEvent(line: string): Lifecycle | undefined {
  if (
    ...
    !/"(?:task_started|task_complete|turn_aborted)"/.test(line)
  )
    return undefined;
  ...
  const turn: LatestTurn = {
    ...
    ...(status === "failed"
      ? { error: { code: "TURN_FAILED", message: "回合执行失败" } }
      : {}),
  };
...
function reduce(previous: Lifecycle | undefined, next: Lifecycle): Lifecycle {
  ...
  // 较早回合的迟到终止事件不能结束当前回合。
  if (
    previous?.turn.status === "inProgress" &&
    next.turn.status !== "inProgress" &&
  ...
  async read(id: string, path?: string | null): Promise<Lifecycle | undefined> {
    ...
    try {
      ...
      while (cursor.search && budget > 0) {
        ...
        while (end > 0) {
          ...
          const event = lifecycleEvent(
            data.subarray(previousNewline + 1, end).toString("utf8"),
          );
          if (event) {
            cursor.latest = event;
        ...
        if (cursor.search) {
          ...
          if (
            search.suffix.length > MAX_RECORD ||
            cursor.pending.length > MAX_RECORD
          )
            throw new Error("LIFECYCLE_RECORD_LIMIT");
      ...
      while (cursor.offset < stat.size && budget > 0) {
        ...
        let begin = 0;
        for (
          let end = data.indexOf(10);
          end >= 0;
          end = data.indexOf(10, begin)
        ) {
          const event = lifecycleEvent(
            data.subarray(begin, end).toString("utf8"),
          );
