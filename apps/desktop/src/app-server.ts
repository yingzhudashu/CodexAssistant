...
function spanContext(parent?: TraceContext): TraceContext {
  return parent
    ? { traceId: parent.traceId, spanId: id(8), parentSpanId: parent.spanId }
...
export class CodexAppServer {
  ...
  setTraceContext(context: TraceContext | undefined): void {
  ...
  async #initialize(): Promise<void> {
    if (Date.now() - this.#lastStartFailureAt < 1_000)
      throw new Error("APP_SERVER_RESTART_BACKOFF");
    this.#plans.clear();
    ...
    try {
      ...
      this.#child.stderr.on("data", (chunk) =>
        this.#onLog?.(
          `app-server stderr: ${String(chunk).trim().slice(0, 1000)}`,
        ),
      );
      this.#child.once("error", (error) => {
        ...
        for (const pending of this.#pending.values()) {
          ...
          pending.reject(new Error("APP_SERVER_SPAWN_FAILED"));
        }
        this.#pending.clear();
      });
      this.#child.once("exit", (code, signal) => {
        ...
        for (const pending of this.#pending.values()) {
          clearTimeout(pending.timer);
          pending.reject(new Error("APP_SERVER_EXITED"));
        }
        this.#pending.clear();
      });
      await this.request(
        "initialize",
        {
          clientInfo: { name: "codex-assistant", version: CLIENT_VERSION },
  ...
  async stop(): Promise<void> {
    ...
    for (const pending of this.#pending.values()) {
      clearTimeout(pending.timer);
      pending.reject(new Error("APP_SERVER_STOPPED"));
    }
    this.#pending.clear();
  }
  ...
  respond(idValue: number | string, result: unknown): void {
    if (!this.#child?.stdin.writable) throw new Error("APP_SERVER_NOT_RUNNING");
    this.#child.stdin.write(
      `${JSON.stringify({ jsonrpc: "2.0", id: idValue, result })}\n`,
    );
  }
  ...
  respondError(idValue: number | string, code: number, message: string): void {
    if (!this.#child?.stdin.writable) throw new Error("APP_SERVER_NOT_RUNNING");
    this.#child.stdin.write(
      `${JSON.stringify({ jsonrpc: "2.0", id: idValue, error: { code, message } })}\n`,
    );
  }
  ...
  async request(
    ...
    this.#child.stdin.write(
      `${JSON.stringify({ jsonrpc: "2.0", id: idValue, method, params })}\n`,
    );
    return response.finally(() =>
      this.#onSpan?.({
        ...context,
        name: `app_server.${method.replace(/[^A-Za-z0-9_.-]/g, "_")}`,
        startedAt,
  ...
  async listThreads(): Promise<AppServerThread[]> {
    ...
    for (const archived of [false, true]) {
      ...
      do {
        ...
        threads.push(
          ...rows.filter((row): row is AppServerThread =>
            Boolean(
              row &&
              typeof row === "object" &&
              typeof (row as AppServerThread).id === "string",
            ),
          ),
        );
    ...
    return [
      ...new Map(
        threads.map((thread) => [
          ...
          {
            ...thread,
            ...(this.#threadStatuses.has(thread.id)
              ? { status: this.#threadStatuses.get(thread.id) }
              : {}),
          },
        ]),
      ).values(),
  ...
  async readThread(idValue: string): Promise<unknown> {
    return this.request("thread/read", { threadId: idValue });
  }
  async listTurns(idValue: string, cursor?: string): Promise<unknown> {
  ...
  latestTurn(idValue: string): unknown | undefined {
    return this.#latestTurns.get(idValue);
  }
  async listItems(idValue: string, cursor?: string): Promise<unknown> {
    return this.request("thread/items/list", {
      threadId: idValue,
      limit: 100,
      sortDirection: "desc",
      cursor: cursor ?? null,
  ...
  planFor(threadId: string): AppServerPlanStep[] | undefined {
    return this.#plans.get(threadId)?.steps;
  }
  planRevision(threadId: string): number {
    return this.#plans.get(threadId)?.revision ?? 0;
  }
  async getGoal(idValue: string): Promise<AppServerGoal | undefined> {
    // 官方明确返回 null 才表示没有 Goal；RPC 失败交给调用方显示 stale，不能假装成功。
    const result = (await this.request("thread/goal/get", {
      threadId: idValue,
    })) as { goal: AppServerGoal | null };
    return result.goal ?? undefined;
  }

  #handleLine(line: string): void {
    if (line.length > MAX_JSON_LINE) {
      this.#onLog?.("app-server response exceeded 2 MiB");
      return;
    }
    ...
    try {
      ...
      this.#onLog?.("app-server emitted invalid JSON");
      return;
    }
    if (
      message.method &&
      ...
      if (message.id !== undefined) {
        ...
        return; // 双向JSON-RPC的请求ID属于各自命名空间，不能误匹配本端等待表。
      } else
        this.#onNotification?.({
          method: message.method,
          params: message.params as Record<string, unknown>,
    ...
    if (message.method === "thread/status/changed") {
      ...
      if (
        ...
        this.#threadStatuses.set(
          params.threadId,
          params.status as AppServerThread["status"],
        );
      return;
    }
    if (
      ...
      if (
        ...
        this.#latestTurns.set(params.threadId, params.turn);
      return;
    }
    if (message.method === "turn/plan/updated") {
      ...
      if (typeof params.threadId === "string" && Array.isArray(params.plan)) {
        ...
        this.#plans.set(params.threadId, {
          revision: previous + 1,
          steps: params.plan.filter((step): step is AppServerPlanStep =>
    ...
    if (message.error)
      pending.reject(
        new Error(message.error.message ?? "APP_SERVER_RPC_ERROR"),
      );
