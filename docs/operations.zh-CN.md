...
|Android后台通知迟到|先确认已安装2.0.16/code19，再检查通知权限、提醒渠道、电池优化和后台同步状态；按下文Trace区分接收延迟与通知提交延迟|
|服务部署后客户端版本未变|服务器部署不安装客户端；按[升级指引](release.zh-CN.md#客户端升级指引)分别更新，Windows已是2.0.16时无需为本次安卓修复重装|
