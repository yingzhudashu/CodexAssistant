...
历史 `task_started` 不能单独证明当前仍在执行。仅当官方运行态为 `notLoaded` 且最近回合尚未结束时，使用 rollout 文件最近写入时间作为有界活动证据；达到 5 分钟未写入便撤销本机推断的 active，显示 idle/stale 和 `ACTIVE_EVIDENCE_EXPIRED`（运行状态待确认）。历史 latestTurn 仍保留 inProgress，不能伪造 completed、interrupted 或 failed；Goal 为 active 也不能使过期证据继续显示运行中，其他 Goal 状态保持优先。完成和中止事件不需要续期，官方明确报告的运行/等待状态不受此窗口影响。

文件写入是启发式证据，不是进程存活证明：无输出的长命令或无法观测的审批等待超过 5 分钟，也会进入待确认；外部程序修改文件时间可能使证据重新变新。异常未来时间超过本机时间一秒视为无效，亚秒差允许通过以适应 Windows 文件时间精度。这条链路无法观测另一个进程的审批等待标志，也不能仅凭时间证明任务完成。详细等待状态仍依赖官方状态或 Goal，不能宣称所有运行细节都能从本机事件恢复。
