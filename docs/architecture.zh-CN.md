...
通知在Repository归约事件时独立记录变化和revision，不能从可能合并的UI状态推断。首次快照建基线，重连快照补报最终变化；服务按任务合并待发内容，以300ms固定间隔共享系统通知预算，提交后按revision确认。常驻概览包含进行中/待确认数，普通进度声音十秒节流，待确认/完成/失败独立计时，不吞掉首次关键状态提醒；文本始终更新。持续订阅采用有明确用途声明的specialUse前台服务；电池豁免由用户在系统确认，短时CPU锁只覆盖已收到事件的通知处理。设置复用 UpdateChecker，避免每次点击创建 HTTP 连接池。
