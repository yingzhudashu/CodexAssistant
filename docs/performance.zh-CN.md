...
桌面端按线程更新时间、状态、标题和计划修订做缓存；无变化线程不再调用四个详情 RPC。线程详情采用并发上限，单个线程读取失败按缓存重新投影并标记 stale，无缓存则标记 unavailable，不阻塞其他线程。30 分钟普通静默窗口和 6 小时进行中操作窗口每轮重新判断，不能因为元数据未变化而跳过；文件时间续期本身不会触发重复上传。
...
现有可重复执行的基准只有 `npm run perf:server`：在隔离临时 SQLite 中对同一任务发送 1,000 条事件，统计进程内 Fastify inject 延迟。脚本校验 HTTP 响应、事件序号和最终快照，失败退出而不是把错误请求混入正常测量；没有针对耗时阈值的自动失败断言，也不是 1,000 个并发任务或公网压测。正常完成及校验失败均关闭服务并清理本次临时数据库。
...
基准 JSON 包含 Node 版本、操作系统平台、CPU 架构、事件数、ingest p50/p95、单次 tasks 查询耗时和进程 RSS。查询只有一个样本，因此不能解释为查询 p95；RSS 是一次采样，不能解释为内存增长。比较结果时应使用相同机器、运行时和负载，不将历史单次毫秒数作为长期达标结论。
