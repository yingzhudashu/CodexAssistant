...
桌面端按线程更新时间、状态、标题和计划修订做缓存；无变化线程不再调用四个详情 RPC。线程详情采用并发上限，单个线程读取失败按缓存重新投影并标记 stale，无缓存则标记 unavailable，不阻塞其他线程。5 分钟运行证据窗口每轮重新判断，不能因为元数据未变化而跳过；文件时间续期本身不会触发重复上传。
