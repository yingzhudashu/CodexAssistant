...
下载目录由服务部署脚本创建为 `/srv/www/codex-assistant/downloads`，公网入口为 `https://server.example.com/codex-assistant/downloads/`。发布时只上传已经校验哈希的 APK/EXE 和 `manifest.json`，不得上传 keystore、证书私钥或 Token。Android 不依赖任何推送供应商凭据。
