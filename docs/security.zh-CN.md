...
本机文件修改时间仅作为运行证据有效期输入，不随任务或 Trace 上传；该时间可被外部修改，不是可信的进程存活凭据。日志中的安全代码和采集推断不能代替任务实际执行结果。
...
`.gitignore` 排除环境文件、生成目录、日志、数据库和常见签名材料；忽略规则不等于凭据审计。提交前检查暂存差异，保留 Gradle Wrapper 和应用图标等必要二进制资源。`npm run clean` 只处理固定生成路径，拒绝目录链接，不读取或删除仓库外凭据。

提交审查需同时覆盖工作树和所有尚未推送的提交，不能只看最终版本。测试使用合成标识、占位凭据与通用路径；文档只记录匿名验证结果，不保留个人邮箱、用户目录、真实任务标识或对话片段。项目公开下载地址与仓库地址属于发布信息。
