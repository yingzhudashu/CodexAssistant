...
公网反向代理必须保留 `/codex-assistant/` 前缀，并将 `/api/v3/stream` 升级为 WebSocket。`codex-assistant-staging.service` 使用端口 `3241`、状态目录 `/var/lib/codex-assistant-staging`、发布目录 `/opt/codex-assistant-staging/current` 和独立环境文件，发布到 `staging.robotclaw.site`；生产和 staging 不得复用状态文件或 Token。
